...
class SignupFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        view.findViewById<Button>(R.id.btn_registrarusuario).setOnClickListener{
            if (names.text.toString().length == 0 || apellidos.text.toString().length == 0 || correo.text.toString().length == 0|| password.text.toString().length == 0 || telefono.text.toString().length == 0) {
